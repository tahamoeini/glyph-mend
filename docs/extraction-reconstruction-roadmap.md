...
Status: audit-only architecture record. No extraction behavior was changed by this audit.
...
| Raw text/layout preparation | `structured-lines.js`, `extract-worker.js` | Collects block/line/character callbacks, coalesces duplicate/overlapping blocks, estimates body size | Coalescing is destructive; the original structured-text tree is not retained as the page evidence model |
| Reading order | `layout-order.js` and worker helpers | Uses y/x ordering with one two-column signature heuristic | No general region graph, N-column model, RTL model, footnote/sidebar model, or reading-order confidence calibration |
...
| Page reconstruction | `document-ir.js`, worker Markdown helpers | Emits headings, paragraphs, lists, tables, equations, figures, captions/placeholders, and page Markdown | Several structures are inferred directly into Markdown rather than represented as typed, source-linked nodes |
...
| LOSS-02 | Text is normally ordered by y then x; a special case recognizes at most a stable two-column signature. | `orderPageEntries` has one pivot-based heuristic, not a region/reading-order graph. | `layout-order.js`, worker page assembly | `twoColumnSignature` requires one large gap and emits left then right; tests cover stable two-column ordering only. | Build a layout graph with regions, adjacency, full-width separators, sidebars, footnotes, and explicit order evidence. | Yes for reliable structural reconstruction. |
| LOSS-03 | Paragraphs are not represented from a durable line/spacing/indent model; cleanup only joins a narrow class of cross-page continuations. | Blocks/lines are converted to Markdown early, then whitespace heuristics operate on text. | `extract-worker.js`, `cleanup.js` | `joinPageParagraphs` requires no punctuation, a long prior line, lowercase continuation, and a page boundary. | Preserve line boxes, baseline/leading, indentation, and paragraph candidates in layout IR before Markdown reconstruction. | Yes for paragraph and list fidelity. |
| LOSS-04 | Heading detection uses font-size/weight and textual patterns, with conservative fallback levels; PDF structural tags are not used as a canonical hierarchy. | Heading inference is local and heuristic; no document section graph is built. | `extract-worker.js`, `cleanup.js`, `document-ir.js` | `headingFor` and OCR heading helpers; no heading ground-truth fixture or section-tree evaluator. | Add heading candidates with evidence, level confidence, and document-level hierarchy reconciliation. | Yes for navigation and semantic export. |
...
| LOSS-08 | Vector tables require stable horizontal/vertical rules; sparse/merged/span grids are rejected. Other table paths produce raw rows or Markdown without cell-level provenance. | Table detection is split among rule-grid, block-start, text-gap, and OCR heuristics, with no canonical cell model. | `extract-worker.js` (`pageTableFromVectors`, `pageTableFromBlocks`, `ocrTableModel`), `document-ir.js` | Vector-table path rejects spans; OCR table model returns Markdown; `normalizeTableIR` supports spans but current producers often provide empty spans/raw rows. | Produce a cell graph with row/column indices, spans, bboxes, source tokens, confidence, and multipage keys; make Markdown/DOCX adapters consume it. | Yes for table fidelity and semantic editing. |
