...
|---|---|---|---|---|---|---|---|
| `@tesseract.js-data/eng` | English OCR traineddata copied to `dist/tessdata` | npm package metadata: `Apache-2.0` | `eng.traineddata.gz` is a model/data artifact distributed by the package. Upstream training-corpus provenance was not independently re-audited in this step; no separate repository dataset is bundled. | Apache-2.0 permits commercial redistribution subject to notice/license terms. | No copyleft source-disclosure requirement from Apache-2.0. | Manifest `^1.0.0`; lockfile tarball URL + integrity; copied path `4.0.0_best_int/eng.traineddata.gz`. | `APPROVED_WITH_NOTE` |
...
| `tesseract.js-core` (transitive, explicitly copied) | Tesseract WebAssembly core copied into the offline bundle | lockfile license is checked automatically by `license:check` | No model weights. | Redistribution terms follow the package license and notices; treat copied WASM as shipped software. | Determined by the locked package license; no extra model obligation. | Version/license reported by `npm run license:report`; lockfile URL/integrity. | `APPROVED_WITH_NOTE` |
...
- `tesseract.js-core/*.{wasm,wasm.js}` → governed by the locked `tesseract.js-core` package license;
- `tesseract.js/dist/worker.min.js` → governed by `tesseract.js` Apache-2.0;
- `@tesseract.js-data/eng/.../eng.traineddata.gz` → model/data artifact reviewed separately above.
