...
| OCR-only report has zero source visuals | Confirm that the current extraction version is 6, then retry with **Embed source figures and formula fallbacks** enabled. Version 6 retains a rendered source page only when an OCR page has a figure, table, or equation cue, and reports `OCR_ONLY_PAGES`; it does not claim editable reconstruction of raster tables or formulas. |
